...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
				<form class="Huiform" method="post" action="<?php echo U('brand/brandlist');?>" target="_self">
					<input type="text" class="input-text" style="width:200px" placeholder="品牌名" id="" name="brandname" <?php if(!empty($params['brandname'])): ?>value="<?php echo ($params['brandname']); ?>"<?php endif; ?>>
					<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索品牌</button>
				</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="brand_add('添加品牌','brandadd','','')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加品牌</a></span> <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
			<table class="table table-border table-bordered table-bg table-sort">
				...
				<tbody>
					<?php if(is_array($data)): foreach($data as $key=>$val): ?><tr class="text-c">
						...
						<td><img src="<?php echo ($val['photo']); ?>" height='35'></td>
						<td><a title="编辑" href="javascript:;" onclick="brand_edit('品牌编辑','<?php echo U("brand/brandedit",array("id"=>$val["id"]));?>','','')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  <a title="删除" href="javascript:;" onclick="brand_del(this,<?php echo ($val['id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
