...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
				<form class="Huiform" method="post" action="<?php echo U('goods/goodslist');?>" target="_self">
					<input type="text" class="input-text" style="width:200px" placeholder="商品名" id="" name="goodsname" <?php if(!empty($params['goodsname'])): ?>value="<?php echo ($params['goodsname']); ?>"<?php endif; ?> >
					<input type="text" class="input-text" style="width:200px" placeholder="分类" id="" name="category" <?php if(!empty($params['category'])): ?>value="<?php echo ($params['category']); ?>"<?php endif; ?> >
					<input type="text" class="input-text" style="width:200px" placeholder="品牌" id="" name="brand" <?php if(!empty($params['brand'])): ?>value="<?php echo ($params['brand']); ?>"<?php endif; ?> >
					<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索商品</button>
				</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="<?php echo U('goods/goodsadd');?>" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加商品</a></span> <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
			<table class="table table-border table-bordered table-bg table-sort">
				...
				<tbody>
					<?php if(is_array($data)): foreach($data as $key=>$val): ?><tr class="text-c">
						...
						<td><?php echo ($catelist[$val['category']]); ?></td>
						<td><?php echo ($brandlist[$val['brand']]); ?></td>
						<td><img src="<?php echo ($val['image']); ?>" height='50'></td>
						<td><a title="编辑" href="javascript:;" onclick="goods_edit('商品编辑','<?php echo U("goods/goodsedit",array("id"=>$val["id"]));?>','900','700')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  <a title="删除" href="javascript:;" onclick="goods_del(this,<?php echo ($val['id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
