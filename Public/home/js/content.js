...
/********** 列表页 ************/
	...
	function listTagFn(){
		...
		$('.more').click(function(){
			var index=$(this).parent().index(); if(onOffArr[index]){
...
/************ 详情页  ****************/
	...
	function detail_content_tab(){
		...
		var scrollTop=[];
		//$('.s_d_nav').height($('.s_details .s_dl').height())
...
// 购物车页
	...
	function order1Fn(){
		...
		$('.order1_checkbox_total').click(function(){
			...
			if($('.order1_checkbox_total').find('i').hasClass('checked')){
				onoff=true;
			}else{
				onOff=false;
			}
...
//订单确认页	
	...
	$('.model_select li').hover(function(){
	...
	$('.model_select li').click(function(){
		...
		$(this).parents('.select_c').find('.select_cs').html($(this).text())
	})
